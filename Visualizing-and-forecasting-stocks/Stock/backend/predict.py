...
def predict_stock(symbol, period, sim, future):
    ...
    class Model:
        ...
        forget_bias = 0.1,
            ...
            self.X = tf.compat.v1.disable_eager_execution()(tf.float32, (None, None, size))
            self.Y = tf.compat.v1.disable_eager_execution()(tf.float32, (None, output_size))
            drop = tf.contrib.rnn.DropoutWrapper(
                rnn_cells, output_keep_prob = forget_bias
            )
            self.hidden_layer = tf.compat.v1.disable_eager_execution()(
